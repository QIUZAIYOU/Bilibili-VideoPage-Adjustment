...
`01.09 11:10`：修复「bangumi」页面播放「电影」类型视频时自动选择「播放器默认模式」失败的问题。设置里新增「脚本执行失败自动刷新重试」选项。

### 2023

`12.28 20:20`：适配最新的「bangumi」页面代码，修复「自动定位」及「点击播放器自动定位」错位的问题，其他积累问题修复。

`11.14 20:58`：调整「自动定位」判定条件，提高定位成功率。

`11.11 21:49`：修复「番剧播放页」自动选择「播放器默认模式」失败的问题。

`11.11 20:59`：修复因数字精度问题导致「自动定位」失效的问题。
